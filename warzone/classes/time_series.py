...
from warzone.utils.class_functions import MU_LST, SUM_LST, _get_sessions, _get_group_stats
...
class TSanalysis:
    ...
    def __init__(self, our_doc_filter: DocumentFilter, other_doc_filter: DocumentFilter, lower: Optional[float] = 0.159,
        ...
        if quantile_lst is None:
            self._quantile_lst = MU_LST

        self._median_lst = median_lst
        if median_lst is None:
            self._median_lst = MU_LST
        ...
        if use_sessions:
            for dic, df in [(self._our_dic, our_df), (self._other_dic, other_df)]:
                temp = _get_sessions(data=df, minutes=session_threshold_time)
                for key, val in temp.items():
                    dic[key] = _get_group_stats(args=(val,) + lst)
        else:
            self._our_dic = {0: _get_group_stats(args=(our_df,) + lst)}
            self._other_dic = {0: _get_group_stats(args=(other_df,) + lst)}
