...
def regression_calcs(df: pd.DataFrame,
                     y: np.ndarray,
                     r2: Optional[Union[float, None]] = None,
                     constant_coef: Optional[Union[float, None]] = None,
                     word_coef: Optional[Union[float, None]] = None,
    ...
    # if word_coef is not None:
    #     result_df = result_df[result_df['Word Coefficient'] > word_coef]
    if confidence_interval_lower is not None:
...
def regress(xtrain: np.ndarray,
    ...
    x_fit = np.linspace(np.floor(np.min(xtrain)), np.ceil(np.max(xtrain)), 2)

    if plotn:
        plt.scatter(xtrain, ytrain, marker='o')
        plt.plot(x_fit, alpha * x_fit + beta, 'r--')
